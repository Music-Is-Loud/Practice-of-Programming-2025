...
//看武器是否有攻击性
bool foundAttactWeapon(CWarrior* w)
{
    for(int i=0;i<w->weaponTotal;i++)
    {
        if(w->weaponOwn[i]->available!=0 and w->weaponOwn[i]->attack!=0) return true;
    }
    return false;
}
...
//out<<"**"<<w2->weaponOwn[i1]->weaponName<<w2->weaponOwn[i2]->weaponId<<" "<<w2->warriorId<<endl;
        ...
        if(foundAttactWeapon(w1)==false and foundAttactWeapon(w2)==false) return 4;
    }
    return -1;
}
//两个司令部战斗
